...
def GetMenu():
    ...
    try:
        ...
        if checkrtmp == 'true':
            ...
            if rtmp_check == 0:
                addLink('[COLOR red][B]RTMP Input is Disabled. Click here to Enable.[/B][/COLOR]','inputstream.rtmp',996,icon,fanarts,'')   
            adaptive_check = CHECK_DB_ENABED('inputstream.adaptive')
            if adaptive_check == 0:
                addLink('[COLOR red][B]InputStream Adaptive is Disabled. Click here to Enable.[/B][/COLOR]','inputstream.adaptive',996,icon,fanarts,'')
    except: pass
